...
namespace Dive
{
	...
	TextureHandle TextureManager::LoadCubemap(const std::wstring& filepath)
	{
		...
		// DirectXTex 유틸리티를 사용하여 텍스처 객체와 SRV를 한 번에 생성
		hr = DirectX::CreateTexture(device, scratchImage.GetImages(), scratchImage.GetImageCount(), metaData, texture.GetAddressOf());
		if (FAILED(hr))
		{
			spdlog::error("큐브맵의 텍스쳐 생성 실패: {}", ErrorUtils::ToVerbose(hr));
			return INVALID_TEXTURE_HANDLE;
		}

		hr = DirectX::CreateShaderResourceView(device, scratchImage.GetImages(), scratchImage.GetImageCount(), metaData, srv.GetAddressOf());
		if (FAILED(hr))
		{
			spdlog::error("큐브맵의 셰이더 리소스 뷰 생성 실패: {}", ErrorUtils::ToVerbose(hr));
			return INVALID_TEXTURE_HANDLE;
		}
