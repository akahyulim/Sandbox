...
float3 CalcuDirLight(float3 worldPos, float3 normal, LightData light)
{
	...
	// Blinn specular
    float3 toEye = frameData.position.xyz - worldPos;
...
float3 CalcuSpotLight(float3 worldPos, float3 normal, LightData light)
{
    ...
    float3 toEye = normalize(frameData.position.xyz - worldPos);
...
*/
    ...
    lightColor += CalcuDirLight(input.WorldPos, normal, lightData);
